...
impl ContentType {
	...
	pub fn from_path(path: &Path) -> ContentType {
		let inferred = infer_mime(path).map(|mime| ContentType::from(mime.as_str()));
		match inferred {
			Some(ct) if ct != ContentType::UNKNOWN => ct,
			_ => ContentType::from_extension(
				path.extension()
					.unwrap_or_default()
					.to_str()
					.unwrap_or_default(),
...
mod tests {
	...
	/// Verifies that `from_path` falls back to the file extension when `infer` returns a
	/// MIME type that maps to `ContentType::UNKNOWN`. This covers PDFs (and other formats)
	/// generated by tools that produce unusual byte patterns at the start of the file.
	#[test]
	fn test_content_type_from_path_unknown_infer_fallback() {
		let mut tmp = tempfile::Builder::new()
			.suffix(".pdf")
			.tempfile()
			.expect("Failed to create temp file");
		std::io::Write::write_all(&mut tmp, b"\x00\x00\x00\x00NOT_ANY_KNOWN_MAGIC")
			.expect("Failed to write bytes");
		assert_eq!(
			ContentType::from_path(tmp.path()),
			ContentType::PDF,
			"from_path should fall back to extension when infer returns UNKNOWN"
		);
	}
}
